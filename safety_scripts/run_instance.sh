...
${SAFETYARTIST_PYTHON_PATH}/python3 ${TOOL_DIR}/safety_main.py "$CATEGORY" "$ONNX_FILE" "$VNNLIB_FILE" "$RESULTS_FILE" "$TIMEOUT"
EXIT_CODE=$?
...
if [ 0 != ${EXIT_CODE} ]; then
        ...
        if [[ ${RESULT_STR} == "" ]]; then
                ${SAFETYARTIST_PYTHON_PATH}/python3 ${TOOL_DIR}/safety_main.py "$CATEGORY" "$ONNX_FILE" "$VNNLIB_FILE" "$RESULTS_FILE" "$TIMEOUT" --TRY_CROWN
        fi
        # we return normally
        exit 0
fi