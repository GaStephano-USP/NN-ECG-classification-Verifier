...
# seven args: 'v1' (version string), tool_scripts_folder, safetyartist_folder, result_csv_file, counterexamples_folder, categories, all|different|first
#
# for example ./run_all_categories.sh v1 ~/repositories/simple_adversarial_generator/safetyArtist_scripts . ./out.csv ./counterexamples "test acasxu" all
...
# check arguments
if [ "$#" -ne 7 ]; then
    echo "Expected 7 arguments (got $#): '$VERSION_STRING' (version string), tool_scripts_folder, safetyartist_folder, result_csv_file, counterexamples_folder, categories, run_which_networks (all|different|first)"
    exit 1
fi
...
if [ ! -d $SAFETYARTIST_FOLDER ] 
then
    echo "SAFETYARTIST directory does not exist: '$SAFETYARTIST_FOLDER'" 
...
echo "Running measurements with vnncomp folder '$SAFETYARTIST_FOLDER' for tool scripts in '$TOOL_FOLDER' and saving results to '$RESULT_CSV_FILE'."
...
do
    INSTANCES_CSV_PATH="${SAFETYARTIST_FOLDER}/benchmarks/${CATEGORY}/instances.csv"
    ...
    do
        ONNX_PATH="${SAFETYARTIST_FOLDER}/benchmarks/${CATEGORY}/${ONNX}"
        VNNLIB_PATH="${SAFETYARTIST_FOLDER}/benchmarks/${CATEGORY}/${VNNLIB}"
	...
	ONNX_PATH="${SAFETYARTIST_FOLDER}/benchmarks/test/test_nano.onnx"
	VNNLIB_PATH="${SAFETYARTIST_FOLDER}/benchmarks/test/test_nano.vnnlib"
	TIMEOUT=120
