...
class FullyConnected(nn.Module):  # Inherits nn.Module do BreastMNISTmodel.py
...
def main():
    ...
    # Configuração dinâmica para carregar o BreastMNIST
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            # Decisão binária baseada na Sigmóide + Limiar 0.5 (assim como no PneumoMNIST)
            preds = (torch.sigmoid(outputs) > 0.5).float()

            correct_test += preds.eq(labels.view_as(preds)).sum().item()
            total_test   += labels.numel()

            # Conversão para arrays unidimensionais para preencher a matriz com segurança
            labels_np = labels.squeeze().long().cpu().numpy()
            preds_np  = preds.squeeze().long().cpu().numpy()

            # Evita quebras caso o batch final tenha tamanho 1
            if labels_np.ndim == 0:
    ...
    # Labels oficiais do BreastMNIST mapeados pelo MedMNIST (0: maligno, 1: benigno)
